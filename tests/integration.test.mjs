...
test('world and character turns, proposals, settings, save/load', async () => {
	...
	try {
		...
		try {
			...
			await post('edit', { id: doheeId, 'player.energy': '1', ...fixedFields });
			await post('conversation', { targetId: doheeId }, true);
			assert.equal(db.prepare('SELECT count(*) AS n FROM events').get().n, 0);
			assert.equal(db.prepare('SELECT energy FROM player_state').get().energy, 1);
			await post('edit', { id: doheeId, 'player.energy': '20', ...fixedFields });
			assert.equal(db.prepare('SELECT energy FROM player_state').get().energy, 20);
			...
			assert.ok(JSON.parse(simulatedEvent.state_changes_json).some((change) => change.path === 'player.BASE.energy' && change.before === 20 && change.after === 18));
			...
			await post('save', { slot: '1' });
			await post('conversation', { targetId: doheeId });
			assert.equal(db.prepare('SELECT energy FROM player_state').get().energy, 16);
			await post('load', { slot: '1' });
			assert.equal(db.prepare('SELECT energy FROM player_state').get().energy, 18);
