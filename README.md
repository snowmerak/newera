...
- `/simulator`는 현재 로어의 인물 수치를 복사해 시작하는 결정적 시뮬레이션 검사 화면입니다. 캐릭터 인스펙터에서 수치를 조정하고 `대화한다`를 누르면 SOURCE, BASE·PALAM·EXP·RELATION·MARK 변화, 의미 사건, 템플릿 문장과 사건 기록을 확인할 수 있습니다. 이 화면의 실행과 편집은 화면 세션 안에서만 유지되며 기존 LLM 플레이 진행과 저장 슬롯은 바꾸지 않습니다. 검증은 `npm run test:simulation`으로 실행합니다.
...
- 진행은 SQLite에 자동 저장됩니다. **각 로어마다 슬롯 1~3**에 스냅샷을 저장·불러올 수 있습니다. 이전 버전의 진행과 슬롯은 최초 실행 시 하나의 `기존 로어`로 연결됩니다.
