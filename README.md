...
- `/lores`는 로어 관리 화면입니다. 로어 생성·설정 및 인물 원본 편집, JSON 가져오기·내보내기를 여기서 합니다. 인물의 초기 수치는 현재 플레이 세션과 별도로 저장되므로 원본을 고쳐도 진행 중 수치가 덮어써지지 않습니다. 이름·설정과 행동 조건은 현재 세션에도 반영됩니다. 내보낸 로어 JSON에는 현재 진행과 저장 슬롯도 포함됩니다.
- `/simulator`는 현재 로어의 실제 상태를 사용하는 결정적 시뮬레이션 검사 화면입니다. 캐릭터 인스펙터에서 수치를 저장하고 `대화한다`를 누르면 SOURCE, PALAM·EXP·RELATION·MARK 변화, 의미 사건, 템플릿 문장과 사건 기록을 확인할 수 있습니다. 실행 결과는 기존 플레이 화면과 로어의 저장 슬롯에도 반영됩니다. 이 행동에는 LLM을 호출하지 않습니다. 검증은 `npm run test:simulation`으로 실행합니다.
- BASE는 등장인물의 체력, TALENT는 0~100의 성향, ABL은 능력, EXP는 누적 경험입니다. 플레이어 체력은 없으며 행동에 체력 비용이나 체력 제한을 두지 않습니다. RELATION은 인물에서 특정 대상으로 향하는 장기 관계이며 현재는 `player` 대상의 6개 수치를 편집합니다. PALAM은 현재 장면의 8개 반응으로, 새 장면이 시작되면 초기화됩니다. MARK는 체크박스와 사용자 정의 태그로 편집합니다.
