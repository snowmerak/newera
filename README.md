...
- 왼쪽에는 **로어의 제목만** 표시됩니다. 로어 하나에 세계관·era 규칙·등장인물·모듈·진행이 묶입니다. 제목을 눌러 로어를 전환하고, 각 로어의 설정은 본문 아래에서 편집합니다. 새 로어는 별도의 빈 진행과 인물 목록으로 시작합니다. 작은 화면에서는 상단 `로어 목록`을 누릅니다.
- 선택한 로어의 `JSON 모듈`에서 세계관과 성인 등장인물 모듈을 설치하고 적용합니다. 세계관 모듈은 한 번에 하나, 인물 모듈은 여러 개를 켤 수 있습니다. [형식과 예시](static/modules/README.md)를 참고하세요. 모듈을 꺼도 해당 로어의 인물 진행과 기억은 유지됩니다.
